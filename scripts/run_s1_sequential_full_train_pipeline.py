...
SOURCE_CANDIDATES = [
    ROOT / "docs" / "XGBoost" / "SETA" / "clean_data" / "clean_dataset_s1.csv",
    ROOT / "SETA" / "S1" / "clean_data" / "clean_dataset_s1.csv",
]
...
VERSION_DESCRIPTIONS = {
    1: "V1 baseline: ใช้ raw/order-time features จาก clean_dataset_s1 เป็นจุดตั้งต้น",
    2: "V2 history: รับ dataset จาก V1 แล้วเพิ่ม customer history และ rolling history แบบ point-in-time",
    3: "V3 interaction: รับ dataset จาก V2 แล้วเพิ่ม business interaction และ group return-rate features",
    4: "V4 segment risk: รับ dataset จาก V3 แล้วเพิ่ม segment/operation risk features",
    5: "V5 compact: รับ dataset จาก V4 แล้วเลือก compact feature set เพื่อลด feature ที่ซ้ำซ้อน/noise",
}
...
def source_path() -> Path:
    ...
    raise FileNotFoundError(f"clean_dataset_s1.csv not found. Checked:\n{candidates}")
...
def build_version_dataset(seq: Any, base_clean: pd.DataFrame, version: int) -> pd.DataFrame:
    out = base_clean.copy()
    for step in range(1, version + 1):
        out = apply_version_feature_engineering(seq, step, out)
    return out
...
def train_full_version(seq: Any, version: int, df: pd.DataFrame, previous_features: list[str]) -> tuple[pd.DataFrame, list[str]]:
    ...
    featured_path = feature_dir / f"df_featured_s1_v{version}.csv"
    used_path = feature_dir / f"used_features_s1_v{version}.csv"
    dropped_path = feature_dir / f"dropped_features_s1_v{version}.csv"
    feature_schema_path = feature_dir / f"feature_schema_s1_v{version}.json"
    train_full_path = feature_dir / f"train_full_sets_s1_v{version}.pkl"
    model_path = model_dir / f"model_s1_v{version}_xgboost.pkl"
    metadata_path = model_dir / f"model_s1_v{version}_metadata.json"
    metrics_path = report_dir / f"metrics_s1_v{version}.csv"
    predictions_path = report_dir / f"full_train_predictions_s1_v{version}.csv"
    ...
    readme = f"""# S1 V{version} - XGBoost Full Training

{VERSION_DESCRIPTIONS[version]}

- Dataset source: `clean_dataset_s1.csv`
- Clean dataset is shared by every version: `{source_path().relative_to(ROOT)}`
...
หมายเหตุ: Accuracy นี้เป็นค่าจากการ train และ predict บนข้อมูลเต็ม 5,000 rows ชุดเดียวกัน ไม่ใช่ holdout test accuracy

## Model Update Policy

Model version นี้รองรับข้อมูลใหม่เมื่อสร้าง feature schema ให้ตรงกับไฟล์ `feature_schema_s1_v{version}.json` แต่ model จะไม่เรียนรู้เองและไม่ทิ้งตัวเก่าแล้วกระโดดเป็นตัวใหม่ ต้อง retrain/tune เป็น version ใหม่ถ้า pattern ของข้อมูลใหม่เปลี่ยน
"""
...
def draw_accuracy_chart(summary: pd.DataFrame) -> None:
    ...
    draw.text((width // 2, 52), "S1 Full-Training Accuracy: V1-V5", font=font(40), fill="#111111", anchor="ma")
    draw.text((width // 2, 98), "No 20% test split: train and predict on full 5,000 rows", font=font(22), fill="#455A64", anchor="ma")
    ...
    out = OUT_ROOT / "images" / "s1_full_training_accuracy_v1_to_v5.png"
    img.save(out)


def write_summary_readme(summary: pd.DataFrame, source: Path) -> None:
    table_rows = []
    ...
    content = f"""# S1 Sequential V1-V5 Full Training

Source clean dataset: `{source.relative_to(ROOT)}`

Flow: each version starts from the same clean source `clean_dataset_s1.csv`, then builds its own feature set:

- `clean_dataset_s1.csv -> V1 features -> V1 model`
- `clean_dataset_s1.csv -> V1+V2 features -> V2 model`
- `clean_dataset_s1.csv -> V1+V2+V3 features -> V3 model`
- `clean_dataset_s1.csv -> V1+V2+V3+V4 features -> V4 model`
- `clean_dataset_s1.csv -> V1+V2+V3+V4 transformations + compact V5 feature set -> V5 model`

ทุก version ใช้ XGBoost และ train ด้วยข้อมูลเต็ม 5,000 rows โดยไม่แบ่ง test 20% ดังนั้น metric ในตารางนี้คือ `full_training_in_sample_no_holdout` ไม่ใช่ holdout test accuracy
...
- V2: เพิ่ม customer history และ rolling history แบบ point-in-time
- V3: เพิ่ม business interaction เช่น category/payment, category/channel, province/payment
- V4: เพิ่ม segment/operation risk เช่น price band, discount band, rating band, province-category risk
- V5: compact feature set ลด feature ที่ซ้ำซ้อน/noise จาก V4
...
def main() -> None:
    ...
    if len(base) != 5000:
        raise AssertionError(f"Expected 5000 rows, got {len(base)}")
    if int(base.isna().sum().sum()) != 0:
        raise AssertionError("Source clean_dataset_s1.csv still has missing/null values")

    base_clean = seq.clean_dataset(base)
    summary_parts: list[pd.DataFrame] = []
    previous_features: list[str] = []

    for version in range(1, 6):
        version_df = build_version_dataset(seq, base_clean, version)
        metrics, features = train_full_version(seq, version, version_df, previous_features)
        summary_parts.append(metrics)
        previous_features = features

    summary = pd.concat(summary_parts, ignore_index=True)
    summary.to_csv(OUT_ROOT / "s1_v1_to_v5_full_train_summary.csv", index=False, encoding="utf-8-sig")
    (OUT_ROOT / "s1_v1_to_v5_full_train_summary.json").write_text(
        summary.to_json(orient="records", force_ascii=False, indent=2),
        encoding="utf-8",
    )
    draw_accuracy_chart(summary)
    write_summary_readme(summary, source)
