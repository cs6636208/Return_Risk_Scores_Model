...
TEST_DATA = ROOT / "docs" / "XGBoost" / "SETB" / "real_data" / "S4" / "real_data_s2.csv"
S2_ROOT = ROOT / "docs" / "XGBoost" / "SETA" / "clean_data" / "S2"
OUT_ROOT = ROOT / "docs" / "XGBoost" / "SETB" / "real_data" / "S4"
TARGET = "is_returned"
...
def test_data_validation(df: pd.DataFrame) -> dict[str, object]:
    ...
    return {
        ...
        "note": "S2 representative external full-dataset test data. Do not use this file for model training.",
    }


def load_used_features(version: int) -> list[str]:
    path = S2_ROOT / f"V{version}" / "features" / f"used_features_s2_v{version}.csv"
    return pd.read_csv(path)["feature"].astype(str).tolist()


def load_model_metrics(version: int) -> dict[str, float]:
    path = S2_ROOT / f"V{version}" / "reports" / f"metrics_s2_v{version}.csv"
...
def evaluate_version(version: int, df_featured: pd.DataFrame) -> dict[str, object]:
    ...
    model_path = S2_ROOT / f"V{version}" / "models" / f"model_s2_v{version}_xgboost.pkl"
    ...
    metrics = {
        "version": f"V{version}",
        "train_dataset": "SETA_S2_clean_dataset_s2",
        "test_dataset": "SETB_S4_real_data_s2",
    ...
    predictions.to_csv(
        report_dir / f"test_predictions_s2_v{version}_on_real_data_s2.csv",
        index=False,
        encoding="utf-8-sig",
    )
    pd.DataFrame([metrics]).to_csv(
        report_dir / f"test_metrics_s2_v{version}_on_real_data_s2.csv",
        index=False,
        encoding="utf-8-sig",
    )
    return metrics
...
def draw_accuracy_chart(summary: pd.DataFrame) -> None:
    ...
    draw.text((width // 2, 48), "S2 Models on real_data_s2.csv", font=font(40, bold=True), fill="#111111", anchor="ma")
    draw.text((width // 2, 96), "Holdout Accuracy vs External Full-Dataset Accuracy", font=font(23), fill="#455A64", anchor="ma")
    ...
    draw.rounded_rectangle((1180, 120, 1225, 145), radius=4, fill="#B0BEC5")
    draw.text((1235, 132), "S2 holdout", font=font(18), fill="#263238", anchor="lm")
    draw.rounded_rectangle((1360, 120, 1405, 145), radius=4, fill="#2E7D32")
    draw.text((1415, 132), "real_data_s2", font=font(18), fill="#263238", anchor="lm")
    image.save(OUT_ROOT / "images" / "s2_models_real_data_s2_test_accuracy_v1_to_v5.png")
...
def write_readme(summary: pd.DataFrame, validation: dict[str, object]) -> None:
    ...
    content = f"""# S2 Model Test on real_data_s2.csv

Test data: `{TEST_DATA.relative_to(ROOT)}`

Train model source: `{S2_ROOT.relative_to(ROOT)}`
...
"""
    (OUT_ROOT / "S2_MODEL_TEST_README.md").write_text(content, encoding="utf-8")
...
def main() -> None:
    ...
    pd.DataFrame([validation]).to_csv(OUT_ROOT / "real_data_s2_validation_for_s2_model_test.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(OUT_ROOT / "s2_v1_to_v5_real_data_s2_test_summary.csv", index=False, encoding="utf-8-sig")
    (OUT_ROOT / "s2_v1_to_v5_real_data_s2_test_summary.json").write_text(
    ...
    comparison.to_csv(OUT_ROOT / "s2_holdout_vs_real_data_s2_comparison.csv", index=False, encoding="utf-8-sig")
