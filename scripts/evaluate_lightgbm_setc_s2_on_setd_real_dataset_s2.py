...
def test_data_validation(df: pd.DataFrame) -> dict[str, object]:
    return {
        ...
        "note": "External full-dataset test for LightGBM SETC S2 models. Do not split this file again.",
    }
...
def evaluate_version(version: int, df_featured: pd.DataFrame) -> dict[str, object]:
    ...
    metrics = {
        ...
        "test_dataset": "LightGBM_SETD_real_dataset_s2",
    ...
    predictions.to_csv(
        report_dir / f"test_predictions_{MODEL_PREFIX}_v{version}_on_real_dataset_s2.csv",
        index=False,
        encoding="utf-8-sig",
    )
    pd.DataFrame([metrics]).to_csv(
        report_dir / f"test_metrics_{MODEL_PREFIX}_v{version}_on_real_dataset_s2.csv",
        index=False,
        encoding="utf-8-sig",
    )
    return metrics
...
def draw_accuracy_chart(summary: pd.DataFrame) -> None:
    ...
    draw.text((width // 2, 48), "LightGBM S2 Models on real_dataset_s2.csv", font=font(38, bold=True), fill="#111111", anchor="ma")
    ...
    draw.text((1180, 132), "S2 holdout", font=font(18), fill="#263238", anchor="lm")
    draw.rounded_rectangle((1325, 120, 1370, 145), radius=4, fill="#2E7D32")
    draw.text((1380, 132), "SETD real_dataset_s2", font=font(18), fill="#263238", anchor="lm")
    image.save(OUT_ROOT / "images" / "lgbm_s2_models_real_dataset_s2_test_accuracy_v1_to_v5.png")
...
def write_readme(summary: pd.DataFrame, validation: dict[str, object]) -> None:
    ...
    content = f"""# LightGBM SETC S2 Model Test on SETD real_dataset_s2.csv
...
def main() -> None:
    ...
    pd.DataFrame([validation]).to_csv(OUT_ROOT / "real_dataset_s2_validation_for_lgbm_s2_model_test.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(OUT_ROOT / "lgbm_s2_v1_to_v5_real_dataset_s2_test_summary.csv", index=False, encoding="utf-8-sig")
    (OUT_ROOT / "lgbm_s2_v1_to_v5_real_dataset_s2_test_summary.json").write_text(
    ...
    comparison.to_csv(OUT_ROOT / "lgbm_s2_holdout_vs_real_dataset_s2_comparison.csv", index=False, encoding="utf-8-sig")
